...
# Node Wrappers -----------------------------------------------------------
class NodePlanRelativeWaypointGUI(NodeGUI):
    def __init__(self):
        super(NodePlanRelativeWaypointGUI,self).__init__('green')

        rospack = rospkg.RosPack()
        ui_path = rospack.get_path('instructor_plugins') + '/ui/action_waypoint.ui'

        self.title.setText('PLAN TO WAYPOINT ACTION')
...
class NodePlanRelativeWaypoint(Node):
    def __init__(self,name,label,waypoint_name,vel,acc,tfl):
        L = 'PLAN RELATIVE TO\\n ['+waypoint_name+']'
        super(NodePlanRelativeWaypoint,self).__init__(name,L,'#26A65B')
        self.command_waypoint_name = waypoint_name
    ...
    def make_service_call(self,request,*args):
        ...
        try:
            rospy.wait_for_service('/costar/PlanToPose')
        ...
        try:
            pose_servo_proxy = rospy.ServiceProxy('/costar/PlanToPose',ServoToPose)
            ...
            if 'FAILED' in str(result.ack):
                ...
                rospy.logwarn('Single Planned Move Finished')
