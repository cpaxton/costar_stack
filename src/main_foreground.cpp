...
//    if( exists_test(out_path+"binary.model") == false || recompute_svm == true )
//        binary_model = TrainBinarySVM(svm_path, out_path, 1, 2, CC_M, true);
//    else
//        binary_model = load_model((out_path+"binary.model").c_str());
...
//                    triple_pooler.InputSemantics(multi_models[ll], ll, true, false);
//                    triple_pooler.InputSemantics(binary_model, ll, true, false);
                    triple_pooler.InputSemantics(binary_models[ll], ll, true, false);
...
model* TrainMultiSVM(std::string fea_path, std::string out_path, int l1, int l2, float CC, bool tacc_flag)
{
    ...
    piece_inds.push_back(std::pair<int, int> (0, 90000));
...
model* TrainBinarySVM(std::string fea_path, std::string out_path, int l1, int l2, float CC, bool tacc_flag)
{
    ...
    model* cur_model = train(&train_prob, &param);
    save_model((out_path+"binary.model").c_str(), cur_model);
