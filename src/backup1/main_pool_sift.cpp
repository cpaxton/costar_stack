...
std::vector<cv::Mat> SIFTPooling(const MulInfoT &data, cv::SiftFeatureDetector *sift_det, cv::SiftDescriptorExtractor * sift_ext, 
...
std::vector<cv::Mat> PoolSIFT(const MulInfoT &data, cv::SiftFeatureDetector *sift_det, cv::SiftDescriptorExtractor * sift_ext, 
                    cv::flann::Index &sift_tree, int sift_code_len, int feaK, const std::vector< boost::shared_ptr<Pooler_L0> > &pooler_set, const cv::Mat &atlas, int MODEL_MAX = 100)
{
    // should prepare data with img and map2d
    cv::Mat cur_rgb = data.img;
    cv::Mat cur_map2d = data.map2d;
    ...
    std::vector<cv::Mat> pool_sift_fea(MODEL_MAX+1);
    for( size_t k = 0 ; k < active_idx.size() ; k++ )
    {
        cv::Mat cur_sift_descr;
        sift_ext->compute(cur_gray, siftKey_set[active_idx[k]], cur_sift_descr);
        std::vector<cv::Mat> sift_code_vec(cur_sift_descr.rows);
        for(int r = 0 ; r < cur_sift_descr.rows ; r++ )
        {
            cv::normalize(cur_sift_descr.row(r), cur_sift_descr.row(r));
            sift_code_vec[r] = KNNEncoder(cur_sift_descr.row(r), sift_tree, sift_code_len, feaK);
//            std::cerr << sift_code_vec[r] << std::endl;
//            std::cin.get();
        }
        cv::Mat cur_sift_code;
        cv::vconcat(sift_code_vec, cur_sift_code);
        
        PreCloud(data_set[active_idx[k]], -1, false);
//        std::cerr << data_set[active_idx[k]].rgb.rows << " " << data_set[active_idx[k]].down_cloud->size() << std::endl;
	std::vector<cv::Mat> pool_fea_vec;
	for( size_t j = 1 ; j < pooler_set.size() ; j++ )
    	{
            cv::Mat cur_final = pooler_set[j]->PoolOneDomain(data_set[active_idx[k]].rgb, cur_sift_code, 1, false);
            pool_fea_vec.push_back(cur_final);
//            cv::Mat cur_final1 = pooler_set[j]->PoolOneDomain(cur_sift_descr, main_fea[0], 2, false);
//            cv::Mat cur_final2 = pooler_set[j]->PoolOneDomain(cur_sift_descr, main_fea[1], 2, false);
//            pool_fea_vec.push_back(cur_final1);
//            pool_fea_vec.push_back(cur_final2);
    	}
	cv::hconcat(pool_fea_vec, pool_sift_fea[active_idx[k]]);
        //cv::Mat cur_final1 = genericPooler.PoolOneDomain(cur_sift_descr, main_fea[0], 2, false);
        //cv::Mat cur_final2 = genericPooler.PoolOneDomain(cur_sift_descr, main_fea[1], 2, false);
        //cv::hconcat(cur_final1, cur_final2, sift_pool_fea[active_idx[k]]);
    }
    
    return pool_sift_fea;
}
...
//    std::vector< boost::shared_ptr<Pooler_L0> > sift_pooler_set(4+1);
...
//    sift_pooler_set[4]->LoadSeedsPool(sift_path+"dict_sift_L0_200.cvmat");
    ...
    int feaK, sift_len;
    LoadSeedsHigh(sift_path+"dict_sift_L0_200.cvmat", sift_tree, sift_seeds, feaK, ratio);
    sift_len = sift_seeds.rows;
    std::cerr << "FeaK: " << feaK << std::endl;
    std::cerr << "SIFT Len: " << sift_len << std::endl;
...
//                std::vector<cv::Mat> cur_final_vec = SIFTPooling(tmp_data, sift_det, sift_ext, hie_producer, sift_pooler_set, cur_atlas, 1);
//		cv::Mat lab_fea;
//		{
...
//		    lab_fea = multiPool(lab_pooler_set, tmp_data, main_fea);
//			
//		}
//		cv::Mat cur_final;
//		cv::hconcat(cur_final_vec[1], lab_fea, cur_final);
                std::vector<cv::Mat> cur_final_vec = PoolSIFT(tmp_data, sift_det, sift_ext, sift_tree, sift_len, feaK, lab_pooler_set, cur_atlas, 1);
...
//                cv::waitKey();
                ...
                std::vector<cv::Mat> cur_final_vec = PoolSIFT(cur_data, sift_det, sift_ext, sift_tree, sift_len, feaK, 
                                                            lab_pooler_set, cur_atlas, max_model_id);
//                std::vector<cv::Mat> cur_final_vec = SIFTPooling(cur_data, sift_det, sift_ext, hie_producer, sift_pooler_set, cur_atlas, max_model_id);
...
//			  cv::Mat cur_final;
//			  cv::hconcat(sift_final, lab_final, cur_final);
		        if( fea_dim > 0 && cur_final.cols != fea_dim )
                        {
                            std::cerr << "Error: fea_dim > 0 && cur_final.cols != fea_dim   " << fea_dim << " " << cur_final.cols << std::endl;
                            exit(0);
